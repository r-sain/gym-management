...
const sendWhatsAppReminder = (user) => {
  ...
  const message = `Hi *${user.name}*, this is a reminder from *Akhara Gym*. Your membership is *${user.daysLeft === 'Expired' ? 'expired' : `expiring in ${user.daysLeft}`}*. Please visit the gym to renew your plan. Thank You!`;
...
const ExpiryList = ({ users }) => {
  if (!users || users.length === 0) {
    return (
      <div className="w-full flex items-center justify-center py-10">
        <p className="text-slate-500 text-sm">No members expiring soon</p>
      </div>
    );
  }
