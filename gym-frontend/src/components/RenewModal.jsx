...
const RenewModal = ({ isOpen, onClose, onRenew, userName }) => {
  const [formData, setFormData] = useState({ planType: '1M', price: '', startDate: new Date().toISOString().split('T')[0] });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    await onRenew(formData.planType, Number(formData.price), formData.startDate);
    setLoading(false);
    onClose();
  };
