...
const InfoRow = React.memo(({ label, value, icon, field, editable = false, type = 'text', options = [], editData, setEditData, isEditing }) => (
  <div className="flex flex-col gap-1 p-3 bg-slate-900/40 rounded-xl border border-slate-800/50">
    ...
    {isEditing && editable && field ? (
      ...
      <span className="text-sm font-medium text-slate-200">{value || 'Not provided'}</span>
    )}
  </div>
));

const UserDetailsModal = ({ isOpen, onClose, user, onPhotoSaved, onUserUpdated }) => {
      ...
      <div className="bg-card border border-slate-700/50 rounded-[2rem] shadow-2xl w-full max-w-2xl max-h-[95vh] flex flex-col relative overflow-hidden animate-in fade-in zoom-in duration-300">
          ...
          <div className="flex gap-4 items-center">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-full font-bold uppercase tracking-widest">{user.plan?.type} Member</span>
                <span className="text-slate-500 text-[10px] font-bold tracking-wider">BILL NO: {user.billNumber || 'N/A'}</span>
              </div>
            </div>
          </div>
          <button 
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <InfoRow label="WhatsApp Number" value={user.phone} field="phone" editable={true} editData={editData} setEditData={setEditData} isEditing={isEditing} />
            <InfoRow label="Alternate Phone" value={user.alternatePhone} field="alternatePhone" editable={true} editData={editData} setEditData={setEditData} isEditing={isEditing} />
            <InfoRow label="Guardian Name" value={user.guardianName} field="guardianName" editable={true} editData={editData} setEditData={setEditData} isEditing={isEditing} />
            <InfoRow label="Blood Group" value={user.bloodGroup} field="bloodGroup" editable={true} type="select" options={bloodGroupOptions} editData={editData} setEditData={setEditData} isEditing={isEditing} />
            <InfoRow label="Birthdate" value={user.birthdate ? formatDate(user.birthdate) : 'N/A'} field="birthdate" editable={true} type="date" editData={editData} setEditData={setEditData} isEditing={isEditing} />
            <InfoRow label="Address" value={user.address} field="address" editable={true} editData={editData} setEditData={setEditData} isEditing={isEditing} />
          </div>

          <div className="mt-6 pt-6 border-t border-slate-800">
            <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Billing & Plan Info</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
              <div className="flex flex-col">
                <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">Plan Started</span>
                <span className="text-sm font-semibold text-slate-200">{formatDate(user.plan?.startDate)}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">Plan Expiry</span>
                <span className="text-sm font-semibold text-slate-200">{formatDate(user.plan?.endDate)}</span>
              </div>
              ...
              {user.enrollmentFees > 0 && (
                <div className="flex flex-col">
                  <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">Enrollment Fee</span>
                  <span className="text-sm font-semibold text-slate-200">₹{user.enrollmentFees}</span>
                </div>
              )}
            </div>
            ...
            {user.discountReason && (
              <div className="mt-6 p-4 bg-primary/5 border border-primary/10 rounded-2xl">
                <span className="text-[10px] text-primary/60 font-black uppercase tracking-widest block mb-2">Discount Reason / Note</span>
                <p className="text-xs text-slate-300 italic leading-relaxed">"{user.discountReason}"</p>
              </div>
            )}
          </div>
        </div>
